...
from ..adapter import sculpt_face_set_ctrl_shift_click_invoke
...
class LeftMouse(bpy.types.Operator, ManuallyManageEvents):
    ...
    def modal(self, context, event):
        ...
        if is_release:  # 单击
            ...
            # ZBrush-style: isolate when fully visible; else hide face set under cursor (H).
            if (
                event.shift
                and event.ctrl
                and not event.alt
...
def execute_brush_stroke(event):
    ...
    if bpy.app.version >= (5, 1, 0):
        if event.alt:
            ...
            args["brush_toggle"] = (
                "None" if brush_runtime.shift_secondary_active else "SMOOTH"
            )
        else:
            args["mode"] = "NORMAL"
            args["brush_toggle"] = "None"
